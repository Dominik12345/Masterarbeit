...
\section{QCD $\times$ dark QCD} \label{QCDxdQCD}
  ...
  \subsection{Die Standardmodell QCD}
    ...
    $\nfc=6$
    verschiedene Quarkflavour und $\Nc=3$ Colours. Da die QCD 
    ...
    \begin{equation}
     ...
     -g_\s \gamma^\mu t^A_{ij} \mathcal{A}_\mu^A
     -m_f \delta_{ij}
     \right)\psi_j^f -\frac{1}{4} F_{\mu \nu}^A F^{A \,\,\, \mu\nu}
     \label{eq:QCDxdQCD:L_QCD}
    \end{equation}
    geschrieben werden \cite{PDG:QCD}. Dabei stellt $\psi_i^f$ ein Quarkfeld mit 
    Colour $i$ und Flavour ${f\in\{1,2,\ldots,\nfc\}}$ 
    und mit der Masse 
    $m_f$ dar, $g_\s$ ist die Kopplungskonstante der QCD. Die Indizes $i,j\in
    \{1,2,\ldots \Nc\}$ gehören zur fundamentalen Darstellung, die  
    adjungierten Darstellung wird mit ${A\in \{1,2,\dots,\Nc^2-1\} }$ dargestellt. Man bezeichnet 
    das Multiplett 
    $\psi=(\psi_1,\psi_2,\ldots,\psi_{\Nc})^\text{T}$ als 
    ...
    \begin{equation}
      ...
      \mathcal{A}_\mu^A - g_\s f^{ABC} \mathcal{A}_\mu^B \mathcal{A}_\nu^C 
  ...
  \subsection{Dark QCD}
    ...
    $g_\d$ untersuch. Es wird eine Energieskala $\Lambda_\text{dQCD}$  gefunden, 
    ...
    auch bis zu beliebig hohen Energie\-skalen anwendbar und soll hier auf ihr 
    ...
    \begin{equation}
     ...
     -g_\d \gamma^\mu \widetilde{t}^M_{ts} \widetilde{\mathcal{A}}_\mu^M
    ...
    die auftretenden Fixpunkte dahingehend, dass Kopplungskonstanten kleiner 
    ...
    \begin{equation}
     ...
     g_\s t^A_{ji} \mathcal{A}^A_\mu -\frac{1}{2}\left(m_f^\text{S}\right)^2 
     \delta_{ji}
     \right] \phi^f_i \right)^\dagger 
     \left( \left[ \i \delta_{ji} \partial^\mu- 
     g_\s t^A_{ji} \mathcal{A}^{A\,\,\, \mu} -\frac{1}{2}\left(m_f^\text{S}
     \right)^2
     \delta_{ji}
     \right] \phi^f_i \right)  
     \end{equation}
     ...
     g_\d \widetilde{t}^M_{sr} \widetilde{\mathcal{A}}^M_\mu -\frac{1}{2}
     \left(\widetilde{m}_f^\text{S}\right)^2 \delta_{sr}
     \right] \chi^f_r \right)^\dagger
     \left( \left[ \i \delta_{sr}\partial^\mu - 
     g_\d \widetilde{t}^M_{sr} \widetilde{\mathcal{A}}^{M\,\,\, \mu} -
    ...
    \begin{align}
     ...
     -g_\s \gamma^\mu t^A_{ij} \mathcal{A}_\mu^A \delta_{rs}
     -g_\d \gamma^\mu \widetilde{t}^M_{rs} \widetilde{\mathcal{A}}_\mu^M 
     \delta_{ij}
     ...
     g_\s t^A_{ji} \mathcal{A}^A_\mu \delta_{sr} 
     - 
     g_\d \widetilde{t}^M_{sr} \widetilde{\mathcal{A}}^M_\mu \delta_{ji} 
    ...
    Eichwechselwirkungen, es gibt jedoch weitere Arten von Wechselwirkungen 
    bei denen keine Eichfelder beteiligt sind, 3-Skalar-Kopplungen, 
    4-Skalar-Kopplungen und Yukawa-Kopplungen. Im Folgenden werden 
    Bedingungen an die Struktur der Yukawa-Terme hergeleitet und es 
    wird kurz begündet, 
    warum 3- und 4-Skalar-Kopplungen nicht weiter betrachtet werden.
    \begin{description}
    ...
    \end{equation}
    Dies ist genau für eine $SU(3)$ gültig. Da in dieser 
    ...
    \input{abschnitte/QCDxdQCD/fig/Yukawa_Majorana.tex}
    ...
    Yukawa-Kopplungs\-kon\-stan\-ten geändert wird. Eine Möglichkeit dieses Problem 
    ...
    Möglichkeit, den dQCD-Sektor konjugiert zur 
    fundamentalen Darstellung transformieren zu lassen, in der infinitesimalen 
    Transformation 
    \begin{equation}
  ...
  \newpage
    ...
    Dummy-Feld beschrieben werden können und fallen aus dem 
    gleichen Grund wie die 4-Skalar-Kopplung aus der Betrachtung heraus.
