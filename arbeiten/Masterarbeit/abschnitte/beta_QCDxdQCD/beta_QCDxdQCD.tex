...
\section{UV-Fixpunkte der $SU_\text{QCD}\times SU_\text{dQCD}$}\label{beta_QCDxdQCD}
  ...
  und \eqref{eq:beta_QCDxdQCD:Z1} andere Werte annehmen können und deshalb 
  ...
  und als Nullstellen findet man den Gaußschen Fixpunkt
   ...
   \begin{equation}
   \alpha^{*}_\text{tw1}=\left(-\frac{X_1}{Y_1},0\right) \quad ,
   \alpha^{*}_\text{tw2}=\left(0,-\frac{X_2}{Y_2}\right) \quad 
   \end{equation}
   und den vollständig wechselwirkenden Fixpunkt
   \begin{equation}
   \alpha^{*}_\text{vw}=\left(\frac{Z_1X_2-X_1Y_2}{Y_1Y_2-Z_1Z_2} ,
  ...
  \subsection{UV-Verhalten bei $\alpha^{*}_\text{vw}$}
	...
	\item $Y_1 > 0 \land Y_2 < 0$:
	  \begin{enumerate}
	   \item $X_2>0$:
	      Wie schon gezeigt ist $Y_2<0 \land X_2>0$ nicht möglich. Dies gilt 
  ...
  \subsection{UV-Verhalten bei $\alpha^{*}_\text{tw1}$}\label{beta_QCDxdQCD:UV_bei_Fix3}
    ...
    Damit $\alpha^{*}_1=-X_1/Y_1 > 0$, muss $X_1<0$ und $Y_1>0$. Es wurde 
    ...
    die $\alpha_1$-Achse UV-repulsiv bezüglich $\alpha^{*}$ sein. 
    ...
    Teilchenanzahlen und die Komponente $\alpha^{*}_1$. Im Gegensatz zum 
