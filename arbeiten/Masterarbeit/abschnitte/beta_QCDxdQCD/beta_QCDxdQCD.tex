...
%   
  ...
  \subsection{UV-Verhalten bei $\alpha^{*4}$}
    \subsubsection{attraktiver Fixpunkt}\label{beta_QCDxdQCD:fix4:UV}
      ...
      \eqref{tab:beta_QCDxdQCD:Sattelpunkt} verglichen. Es bietet sich an, 
	...
	physikalisch sinnvolle Lösung. 
      ...
      gewählten Darstellung \eqref{tab:beta_QCDxdQCD:Sattelpunkt} nicht 
      möglich ist.
      \end{enumerate}
	 ...
	 \item $X_1<0 \land X_2<0$: \label{Fall1b}
	  ...
	  nach unten, für eine SM-QCD mit $\Nc=3$ und $\nfc = 6$ folgt 
	  $\nfj\leq 4$. Die 
	...
	\item $Y_1 > 0 \land Y_2 < 0$:
	 ...
	 \item $Y_1<0 \land Y_2<0$:
      ...
      entsprechenden Faktor höher liegen.
%       Zunächst wird Fall \ref{Fall1b} um Skalare erweitert, eine obere Grenze 
%       für die Teilchenzahl erhält man wieder aus $X_1<0 \land X_2<0$. Da 
%       Skalare in die Koeffizienten $X_i$ genau mit einem Faktor $4$ relativ 
%       zu den Fermionen eingehen, kann \eqref{eq:beta_QCDxdQCD:Sattelpunkt1b} 
%       leicht für Skalare vereinfacht werden,
%       \begin{equation}
%        \left(\nfj+\frac{1}{4} \nsj \right)^2 + \left( \nfj+\frac{1}{4}\nsj 
%        \right) \left( \nfd +\frac14 \nsd \right)\frac1\Nc 
%        + \frac{11}{2} \left(\nfc+\frac14 \nsc \right)\frac1\Nc < 
%        \left( \frac{11}{2} \right)^2 \quad \land \quad \text{c}\leftrightarrow
%        \text{d} \quad.
%       \end{equation}
      ...
      Einige mögliche Standardmodell nahe Teilchenzahlen sind in Tabelle 
...
%     Damit der 
    ...
    Damit $\alpha^*_1=-X_1 Y_1^{-1} > 0$, muss $X_1<0$ und $Y_1>0$. Es wurde 
